...
<script>
	...
	$(function() {
		...
		$('#user_id').keyup(function() {
			...
			if($(this).val() === '') { //입력하지 않은 경우
				$(this).css('background-color', 'pink'); //input창 배경색을 pink로
				$('#idChk').html('<b style="font-size: 14px; color: red">[아이디는 필수 정보입니다!]</b>');
				chk1 = false;
			}
			//아이디 입력값 유효성 검사 (영문으로만 4~14자 허용.) 정규표현식으로 작성.
			...
			//특정 로직의 실행이 끝날 때까지 기다리지 않고 먼저 코드를 실행.(페이지 전환 없이 통신)
				...
				$.ajax({
					...
					error : function(status, error) { //통신에 실패했을 때 실행할 내용
						console.log('통신 실패!');
		...
		//회원 가입 처리를 해 주지 말아야겠지?
		$('#signup-btn').click(function() {
			
			if(chk1 && chk2 && chk3 && chk4) { //연산순서 왼쪽 -> 오른쪽 (chk1 && chk2먼저 그 다음 && chk3)
				...
				const user = { //UserVO의 변수명과 프로퍼티가 같아야 함
						"account" : id, //객체 리터럴 방식으로 작성
						"password" : pw,
						"name" : name
				}; //아직 자바스크립트임 (JSON 아님)
				...
				$.ajax({
					type : 'POST',
					url : '/user/',
					contentType : 'application/json', //headers는 여러개를 쓸 때 열어서 사용함.
					dataType : 'text',
		...
		$('#signInId').keyup(function() {
			if($(this).val() === '') {
				...
				$(this).css('background-color', 'pink');
				$('#idCheck').html('<b style="font-size: 14px; color: red">[영문, 숫자로 4-14로 작성!]</b>');
				chk1 = false;
			} else {
				$(this).css('background-color', 'aqua');
		...
		$('#signInPw').keyup(function() {
			if($(this).val() === '') {
				$(this).css('background-color', 'pink');
				$('#pwCheck').html('<b style="font-size: 14px; color: red">[비밀번호는 쓰세요!]</b>');
				chk2 = false;
		...
		//chk1, chk2를 재활용해서 쓰면 된다.
		//ajax를 이용힌 비동기 방식으로 로그인을 처리할 예정이다.
				...
				$.ajax({
					...
					success : function(data) {
						if(data === 'idFail') {
							...
							$('#signInPw').val('');
							$('#signInId').foucus(); //커서를 이동시키고, 스크롤도 해당 위치로 이동시키는 함수.
