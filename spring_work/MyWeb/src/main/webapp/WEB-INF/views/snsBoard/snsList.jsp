	...
	<script>
		$(function() {
			...
			function regist() {
				...
				const user_id = '${sessionScope.login.userId}'; //sessionScope는 생략 가능
				...
				console.log(data[0].files[0]); //몇번째 파일인지.

				//data[index] -> 파일 업로드 버튼이 여러 개 존재할 경우 요소의 인덱스를 지목하여 가져오는 방법.
				//우리는 요소를 id로 취득했기 때문에 하나만 찍히지만, class이름 같은것으로 지목하면 여려개가 취득된다.
				//files[index] -> 파일이 여러개 전송되는 경우, 몇 번째 파일인지를 지목.
				//우리는 multiple 속성을 주지 않았기 때문에 0번 인덱스 밖에 없는 것이다. (업로드가 하나밖에 안됨)
				//파일이 여러개면 반복문 처리

				//FormData 객체에 사용자가 업로드한 파일의 정보들이 들어있는 객체에 전달.
				formData.append('file', data[0].files[0]); //뒤에 추가함
				//content(글 내용) 값을 얻어와서 폼 데이터에 추가
				const content = $('#content').val();
				formData.append('content', content); //위에꺼 변수 선언 안하고 괄호안에 한번에 작성해도 됨.

				//비동기 방식으로 파일 업로드 및 게시글 등록을 진행.
				$.ajax({
					url : '<c:url value="/snsBoard/upload" />',
					type : 'post',
					data : formData, //폼데이터 객체를 넘긴다.
			...
			function getList(page, reset) {
				if (reset === true) {
					str = ''; //화면 리셋 여부가 true라면 str변수를 초기화. 댓글 작성했던 것과 같은 로직.
				}
				...
				$.getJSON(
							...
							function(list) {
								...
								for (let i = 0; i < list.length; i++) {
									...
									str += "<a href='" + list[i].bno + "'>"; ///////추가 (글번호를 묻혀서 추가)
									str += "<img src='display?fileLoca=" + list[i].fileloca + "&fileName=" + list[i].filename + "'>"; //이미지 태그 자체 요청 보내기 가능(display라는 요청을 컨트롤러로 보냄)
									str += "</a>" ////////추가
									str += "</div>";
			...
			$('#contentDiv').on('click', '.image-inner a', function(e) {
					...
					$.getJSON(
						...
						function(data) { //통신이 성공 했다면 진행 할 함수
						console.log(data);
			...
			//비동기 방식으로 삭제를 진행해 주세요. (삭제 버튼은 여러개)
			//서버쪽에서 권한을 확인하기. (작성자와 로그인 중인 사용자의 id를 비교해서)
			...
			$(window).scroll(function() {
				...
				if(Math.round($(window).scrollTop()) === $(document).height() - $(window).height()) {
					...
					//목록 불러오기의 sql문을 페이징 쿼리를 사용해서 작성을 해주면 된다.
					//사용자의 스크롤이 바닥에 닿았을 때, 페이지 변수의 값을 하나 올리고
					//getList(false)를 주어서 누적해서 계속 열어 주면 된다.
					//게시글을 몇 개씩 불러 올지는 페이징 알고리즘에서 알아서 정해주면 된다.
		            getList(++page, false);
				}
		...
		function readURL(input) {
			if (input.files && input.files[0]) {
				...
				reader.readAsDataURL(input.files[0]); //첫번째 사진
